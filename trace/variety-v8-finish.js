"use strict";
/* TRACE v8.4 release gate: stable scope bounds, executable orchestration patches, startup assertions. */
(()=>{
  const priorCompose=globalThis.compose;
  const priorVariant=globalThis.v8PlanVariant;
  const priorApply=globalThis.applyPlan;
  ...
  function planScope(p){
    const live=scopeObject(),saved=p?.trace_scope;
    if(saved&&Number.isFinite(saved.start)&&Number.isFinite(saved.end))return saved;
    return{kind:live.kind,label:live.label,start:live.start,end:live.end};
  }
  function boundPlan(plan){
    const p=clone(plan),s=planScope(p),songEnd=beats(state.composition),lo=clamp(+s.start,0,songEnd),hi=clamp(+s.end,lo,songEnd);
    p.trace_scope={kind:s.kind||"song",label:s.label||"SCOPE",start:lo,end:hi};
    ...
    p.note_loops=(p.note_loops||[]).map(x=>{
      ...
      y.repeats=boundRepeats(y.start,y.step,y.duration,hi,64);return y;
  ...
  globalThis.applyPlan=function(base,plan,promptId,preview=true){return priorApply(base,boundPlan(plan),promptId,preview)};
  ...
  function selfTest(){
    ...
    releaseAssert(typeof globalThis.applyPlan==="function","bounded apply missing");
    ...
    const probe={summary:"probe",interpretation:[],dimensions:[],alternatives:[],tempo:null,track_changes:[{track:1,name:"PROBE",kind:"lead",wave:"square",program:80,reason:"probe"}],clear_ranges:[],note_loops:[],drum_loops:[],effects:[],sample_assignments:[],trace_scope:{kind:"song",label:"PROBE",start:0,end:Math.min(4,beats(state.composition))}};
